  ...
  useEffect(() => {
    ...
    if (!query) {
      setResults(smartRecs.map(loc => ({
        location: loc,
        similarity: (loc.matchScore || 80) / 100
      })));
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2">Tìm thấy {results.length} địa điểm phù hợp</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="hidden lg:block lg:col-span-1 space-y-6">
            ...
            {/* Map Preview Sticky */}
            <div className="sticky top-24 bg-white rounded-xl shadow-sm border border-slate-100 p-4 h-[300px]">
              <h3 className="font-bold mb-4 text-sm text-slate-700">Xem trên bản đồ</h3>
          ...
          <div className="lg:col-span-3">
            {loading ? (
              ...
              <div className="space-y-6">
                {results.map((item) => (
                  <div key={item.location.id} className="relative group">
                    ...
                    <LocationCard
                      ...
                      onClick={() => window.location.href = `/detail/${item.location.id}`}
                    />
                  </div>
                ))}
              </div>
