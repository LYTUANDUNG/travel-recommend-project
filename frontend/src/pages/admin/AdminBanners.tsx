    ...
    const handleAdd = (e: React.FormEvent) => {
        ...
        api.client.post('/banners', payload).then(res => {
            if (res.data.success) {
                resetForm();
                loadBanners();
                alert(editingId ? "Cập nhật banner thành công!" : "Lưu banner mới thành công!");
            } else {
                alert("Lỗi lưu banner: " + (res.data.message || "Không xác định"));
            }
        }).catch(err => {
            console.error("Lỗi lưu banner:", err);
            alert("Lỗi kết nối khi lưu banner: " + (err.response?.data?.message || err.message));
        });
    };

    const handleDelete = (id: number) => {
        if (!confirm("Bạn có chắc chắn muốn xóa banner này?")) return;
        api.client.delete(`/banners/${id}`).then(res => {
            if (res.data.success || res.status === 200) {
                loadBanners();
                alert("Đã xóa banner thành công!");
            } else {
                alert("Lỗi khi xóa banner: " + (res.data.message || "Không xác định"));
            }
        }).catch(err => {
            console.error("Lỗi xóa banner:", err);
            alert("Lỗi kết nối khi xóa banner: " + (err.response?.data?.message || err.message));
        });
    };
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm">
                    <h2 className="text-xl font-bold mb-6 flex items-center gap-2"><Plus className="w-5 h-5 text-orange-500" /> Thêm Banner mới</h2>
                    <form onSubmit={handleAdd} className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Tiêu đề</label>
                            <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl outline-none focus:border-orange-500" placeholder="Chương trình ưu đãi..." />
                        </div>
                        <div>
                            ...
                            <div className="flex relative">
                                ...
                                <input type="text" value={imageUrl} onChange={e => setImageUrl(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl outline-none focus:border-orange-500" placeholder="https://" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex relative">
                                ...
                                <input type="text" value={link} onChange={e => setLink(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl outline-none focus:border-orange-500" placeholder="/explore" />
                            </div>
                        </div>
                        <div className="flex gap-3 mt-4">
                            <button type="submit" className="flex-1 py-4 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-colors shadow-lg shadow-orange-100 dark:shadow-none">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {loading ? <div className="text-center py-20"><div className="animate-spin w-8 h-8 border-2 border-orange-600 border-t-transparent mx-auto rounded-full"></div></div> : 
                        ...
                        banners.map(b => (
                            <div key={b.id} className="flex flex-col sm:flex-row gap-4 bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm items-center">
                                ...
                                <div className="flex flex-col gap-2">
                                    <button onClick={() => handleEdit(b)} className="w-12 h-12 flex items-center justify-center bg-orange-50 dark:bg-orange-900/30 text-orange-500 rounded-2xl hover:bg-orange-500 hover:text-white transition-colors">
