...
export default function Explore() {
    ...
    if (isMapOnly) {
        return (
            <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pt-24 pb-10 font-sans">
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pt-20 pb-10 font-sans">
            <div className="max-w-[1680px] mx-auto px-4 md:px-8">
                <div className="flex flex-col lg:flex-row gap-6 items-start">
                    ...
                    <div className="w-full lg:w-[46%] flex flex-col gap-4">
                        {/* Search header card */}
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/70 dark:border-slate-800 p-5 shadow-sm relative overflow-hidden">
                            ...
                            <div className="relative z-10 space-y-4">
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm mt-1">Tìm kiếm địa điểm du lịch, ẩm thực và khách sạn trên bản đồ trực quan.</p>
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-14 pr-32 py-3.5 bg-slate-50 dark:bg-slate-800 rounded-2xl outline-none border border-transparent focus:border-orange-500 focus:bg-white text-base font-bold transition-all shadow-inner text-slate-700 dark:text-slate-100"
                                    />
                                    <button 
                                        onClick={() => setSearchQuery(searchInput)}
                                        className="absolute right-2 top-1/2 -translate-y-1/2 px-6 py-2.5 bg-[#f97316] hover:bg-[#ea580c] text-white rounded-xl font-bold text-sm transition-all shadow-md shadow-orange-500/25"
                                    >
                                        Tìm kiếm
                                    </button>
                                </div>
                        ...
                        {/* Smart filters panel */}
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/70 dark:border-slate-800 p-4 shadow-sm">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {/* Tỉnh thành */}
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                                        <MapPin className="w-3.5 h-3.5 text-orange-500" /> Khu Vực Tỉnh Thành
                                    </label>
                                    <select
                                        value={provinceFilter}
                                        onChange={(e) => setProvinceFilter(e.target.value)}
                                        className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl font-bold text-sm text-slate-700 dark:text-slate-300 outline-none border border-transparent focus:border-[#f97316] transition-all cursor-pointer"
                                    >
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                                        <SlidersHorizontal className="w-3.5 h-3.5 text-orange-500" /> Ngân Sách
                                    </label>
                                    <select
                                        value={priceFilter}
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                                        <SlidersHorizontal className="w-3.5 h-3.5 text-orange-500" /> Loại Hình
                                    </label>
                                    <select
                    ...
                    <div className="w-full lg:flex-1 lg:sticky lg:top-24 h-[calc(100vh-132px)] min-h-[560px] rounded-[2rem] overflow-hidden shadow-sm border border-slate-200/70 dark:border-slate-800 bg-white dark:bg-slate-900 p-2">
                        <MapView 
